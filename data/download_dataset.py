...
=============================================================================
 ...
 OUTPUT STRUCTURE:
   ...
   │   └── test/                   # 15% of images
   │       └── ...
   └── class_mapping.json          # {0: "Apple___Apple_scab", ...}
=============================================================================
"""
...
# CONFIGURATION
# =============================================================================
# These constants define the dataset split ratios and image validation rules.
# TRAIN_RATIO + VAL_RATIO + TEST_RATIO must equal 1.0

TRAIN_RATIO = 0.70   # 70% of data for training
VAL_RATIO = 0.15     # 15% of data for validation (hyperparameter tuning)
TEST_RATIO = 0.15    # 15% of data for final evaluation

MIN_IMAGE_SIZE = 10   # Minimum width/height in pixels (rejects tiny/corrupt images)
RANDOM_SEED = 42      # For reproducible splits across runs

# Kaggle dataset identifier — this is the official PlantVillage dataset
KAGGLE_DATASET = "abdallahalidev/plantvillage-dataset"


def download_from_kaggle(raw_dir: str) -> str:
    """
    Downloads PlantVillage dataset from Kaggle using the Kaggle API.

    HOW IT WORKS:
      1. Checks if Kaggle API credentials exist (in ~/.kaggle/kaggle.json or .env)
      2. Uses the kaggle CLI to download and extract the dataset
    ...
    RETURNS:
      str: Path to the directory containing class subdirectories

    RAISES:
      SystemExit: If Kaggle API is not configured
    """
    try:
        ...
        # If Kaggle API isn't installed or configured, provide helpful instructions
        print("\n" + "=" * 70)
    ...
    # Using python -m kaggle ensures we use the correct environment even if kaggle isn't on PATH
    os.system(f'{sys.executable} -m kaggle datasets download -d {KAGGLE_DATASET} -p {raw_dir} --unzip --force')

    # PlantVillage dataset typically extracts to a 'color' subdirectory
    # containing the actual class folders
    possible_paths = [
    ...
    for path in possible_paths:
        if os.path.isdir(path):
            # Check if this directory contains class subdirectories (not just files)
...
def validate_image(image_path: str) -> bool:
    """
    Validates a single image file to ensure it's readable and not corrupt.

    VALIDATION CHECKS:
      1. File exists and has non-zero size
      2. Can be opened by PIL (valid image format)
      3. Meets minimum dimension requirements
      4. Has valid color channels (RGB or convertible to RGB)

    PARAMETERS:
      image_path (str): Full path to the image file

    RETURNS:
      bool: True if image is valid, False if corrupt/invalid
    """
    try:
        with Image.open(image_path) as img:
            # verify() checks for file corruption without loading full image
            img.verify()

        # Re-open because verify() makes the image unusable
        with Image.open(image_path) as img:
            width, height = img.size
            # Reject images that are too small (likely corrupt or placeholder)
            if width < MIN_IMAGE_SIZE or height < MIN_IMAGE_SIZE:
                return False
            return True
    except Exception:
        # Any exception means the image is unreadable
        return False


def create_splits(source_dir: str, output_dir: str):
    """
    Splits the dataset into train/val/test sets with stratified sampling.

    ALGORITHM:
      1. Enumerate all class subdirectories in source_dir
      2. For each class, list all valid images
      3. Shuffle images with fixed random seed (reproducibility)
      4. Split into train (70%), val (15%), test (15%)
      5. Copy images to output_dir/train/<class>/, val/<class>/, test/<class>/

    STRATIFIED SAMPLING ensures each class has proportional representation
    in all three splits, which is critical for balanced evaluation.

    PARAMETERS:
      source_dir (str): Path to directory containing class subdirectories
      output_dir (str): Path where train/val/test splits will be created

    MATHEMATICAL BASIS:
      For N images in a class:
        train_count = floor(N × 0.70)
        val_count   = floor(N × 0.15)
    ...
    # Set seed for reproducible shuffling across runs
    random.seed(RANDOM_SEED)

    # Discover all class directories (each subdirectory = one disease class)
    classes = sorted([
    ...
    class_mapping = {}  # Will store {index: class_name}
    ...
    for idx, class_name in enumerate(tqdm(classes, desc="   Processing classes")):
        ...
        # Validate each image and filter out corrupt ones
        valid_files = []
        for f in all_files:
...
def print_class_distribution(output_dir: str):
    """
    Prints detailed class distribution for each split.

    Shows the number of images per class in train/val/test,
    helping identify any class imbalance issues that might
...
if __name__ == "__main__":
    ...
    if args.manual:
        # Manual mode: user already downloaded the dataset
